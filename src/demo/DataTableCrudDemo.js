...
const DataTableCrudDemo = () => {
  ...
  const saveProduct = () => {
    ...
    if (product.name.trim()) {
      ...
      console.log("new product", _products);
      setProductDialog(false);
      setProduct(emptyProduct);
    }
  };
  ...
  //Showing saved data 
  const experimentDetails = experiments.map((item, idx) => {
    try {
      return (
        <>
          <h1>{item.time}</h1>
          ...
          <Button
            ...
            onClick={() => saveshow(idx)} //Assigns each saved data with a particular index
          />
        </>
      );
    } catch {
      console.log("errorin 316");
    }
  });

  function saveshow(idx) {
    navigate("/saveddata", { state: experiments[idx].product });//navigating saved data to a new page
  }
